...
// A morph target may only refine a semantic the primitive itself declares, so
// the base NORMAL is part of a valid fixture rather than decoration.
const normals = new Int8Array([0, 0, 127, 0, 0, 127, 0, 0, 127]);
const indices = new Uint16Array([0, 1, 2]);
const times = new Float32Array([0, 1]);
const weights = new Uint8Array([0, 255]);

const { bytes: bin, offsets } = packViews([
  positions, normals, morphPositions, morphNormals, indices, times, weights,
]);
...
const document = {
  ...
  accessors: [
    ...
    { bufferView: 6, componentType: 5121, count: 2, type: 'SCALAR', normalized: true },
  ],
  bufferViews: [
    ...
    { buffer: 0, byteOffset: offsets[3], byteLength: morphNormals.byteLength },
    { buffer: 0, byteOffset: offsets[4], byteLength: indices.byteLength },
    { buffer: 0, byteOffset: offsets[5], byteLength: times.byteLength },
    { buffer: 0, byteOffset: offsets[6], byteLength: weights.byteLength },
  ],
...
assert.ok(
  (manifest.extensionsUsed || []).includes('KHR_mesh_quantization'),
  'quantized attributes were written without declaring KHR_mesh_quantization',
);
...
assert.deepEqual(
  ...
  [],
  'the exported quantized asset must validate as glTF 2.0',
);
