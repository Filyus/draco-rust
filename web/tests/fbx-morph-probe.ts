...
const sourceTargets = parsed.scene.rootNodes.flatMap((node: any) => node.meshes || []).reduce((count: number, mesh: any) => count + (mesh.morphTargets?.length || 0), 0);
const previewTargets = scene.meshes.flatMap((mesh: any) => mesh.primitives || []).reduce((count: number, primitive: any) => count + (primitive.morphPositions?.length || 0), 0);
...
const roundtripTargets = reparsed.scene?.rootNodes?.flatMap((node: any) => node.meshes || []).reduce((count: number, mesh: any) => count + (mesh.morphTargets?.length || 0), 0) || 0;
