...
const { Viewer } = await import(pathToFileURL(resolve(here, '..', 'src', 'viewer.ts')).href) as {
  Viewer: typeof ViewerClass;
};
const resources = {
...
function sample(scene: any, time: number) {
  const probe = Object.create(Viewer.prototype) as ViewerClass;
  probe.scene = scene;
  probe.animation = { clipIndex: 0, time: 0 } as any;
  assert.equal(probe.seekAnimation(time), true);
  probe._updateWorldMatrices();
  return scene.nodes.map((node: any) => Array.from(node.world));
}
