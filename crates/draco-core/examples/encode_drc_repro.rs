//! Windows reproducer for an `encode_drc` fuzz finding.
//!
//! A machine-generated copy of `fuzz/fuzz_targets/encode_drc.rs` with its
//! `fuzz_target!` replaced by a `main` that reads one input file, so a crash
//! artifact can be re-run on a toolchain where libFuzzer will not link. It is
//! not a substitute for the campaign and it is not kept in step with the
//! target: regenerate it from the target when a finding needs triaging here.
//!
//! ```text
//! cargo run --release -p draco-core --all-features --example encode_drc_repro -- crash.bin
//! ```
...
fn main() {
    let path = std::env::args().nth(1).expect("usage: encode_drc_repro <input>");
    let data: &[u8] = &std::fs::read(&path).expect("read input");
