...
match decoder_type {
    ...
    // 3 => SequentialNormalAttributeDecoder, which both callers share.
}
```
...
| Folding the wrap transform's two range tests into arithmetic | `prediction_scheme_wrap.rs`, tried 2026-08-14 | **+0.9…+1.4% slower**, and instructive about why. The shape looked identical to `mod_max`, which had just paid −6.6%: two mutually exclusive thresholds on a decoded value, on a function worth 7.2% of mesh decode. But there was no branch to remove — LLVM already lowers this `if`/`else if` pair to two `cmov` and an add, 8 instructions with no jump, where the hand-written fold emits two `setcc`, a subtract and an `imul`: 10 instructions with a multiply on the dependency chain. Reading the source does not tell you which shape you have; `rustc --emit asm` on the loop body does, in under a minute. The chase was still worth it: establishing that the two tests really are mutually exclusive is what turned up the two range validations this port had never carried over from upstream, fixed in `76e1933`. |
